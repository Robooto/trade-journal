...
describe('positions rules', () => {
    it('dteRule reports the current DTE and severity', () => {
        ...
        expect(dteRule(makeGroup({ expires_at: '2025-01-25' }), today)).toEqual(expect.objectContaining({ id: 'dte', label: '24 DTE', level: 'warning' }));
        expect(dteRule(makeGroup({ expires_at: '2025-01-15' }), today)).toEqual(expect.objectContaining({ id: 'dte', label: '14 DTE', level: 'alert' }));
    });

    it('profitRule uses percent_credit_received when available', () => {
        expect(profitRule(makeGroup({ percent_credit_received: 55 }))).toEqual(expect.objectContaining({ id: 'profit', label: '55% captured', level: 'alert' }));
    });
    ...
    it('profitRule calculates percent when not provided', () => {
        ...
        expect(profitRule(group)).toEqual(expect.objectContaining({ id: 'profit', label: '40% captured', level: 'warning' }));
    });

    it('lossRule reports warning and alert thresholds', () => {
        expect(lossRule(makeGroup({ percent_credit_received: -200 }))).toEqual(expect.objectContaining({ id: 'loss', level: 'alert' }));
        expect(lossRule(makeGroup({ percent_credit_received: -175 }))).toEqual(expect.objectContaining({ id: 'loss', level: 'warning' }));
    });

    it('ivRankRule includes the current IV rank', () => {
        expect(ivRankRule(makeGroup({ iv_rank: 12 }))).toEqual(expect.objectContaining({ id: 'iv-rank', label: 'IVR 12', level: 'warning' }));
        expect(ivRankRule(makeGroup({ iv_rank: 9 }))).toEqual(expect.objectContaining({ id: 'iv-rank', label: 'IVR 9', level: 'alert' }));
    });
