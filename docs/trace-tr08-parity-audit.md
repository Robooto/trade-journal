...
| Opposing-HIRO balance | Pass, research-only | Selected captures show the pipeline-owned relationship, balance percentage, combined absolute magnitude, and gamma pocket. High-balance markers appear only in the absolute Pressure view. Copy explicitly states that calibration has not established lower movement or direction, and the evidence strip shows the non-scoring prospective checkpoint. |
